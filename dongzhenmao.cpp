...
private:
    ...
    void InitGraphics() {
        ...
        winrt::check_hresult(CreateDirect3D11DeviceFromDXGIDevice(d3dDevice.as<IDXGIDevice>().get(), inspectableDevice.put()));
    ...
    void StartCapture() {
        ...
        winrt::check_hresult(factory->CreateForWindow(m_targetHwnd, winrt::guid_of<winrt::Windows::Graphics::Capture::GraphicsCaptureItem>(), winrt::put_abi(m_item)));

        auto size = m_item.Size();
        // 边界控制：避免目标窗口尺寸返回 0 导致创建 FramePool 报错或卡死
    ...
    void OnFrameArrived(winrt::Windows::Graphics::Capture::Direct3D11CaptureFramePool const& sender, winrt::Windows::Foundation::IInspectable const&) {
        ...
        // ==========================================
        // 关键调整：在此处不再调用 CreateShaderResourceView。
        // 我们只在多线程下传递纹理，把 SRV 转换工作彻底移交给主线程执行。
        // ==========================================
    ...
    void RenderFrame() {
        ...
        // 仅在获取到新纹理时，于主线程（渲染线程）安全重建 SRV，消除多线程底层冲突
        if (newTexture) {
            m_currentSRV = nullptr; // 释放旧帧
            winrt::check_hresult(m_d3dDevice->CreateShaderResourceView(newTexture.get(), nullptr, m_currentSRV.put()));
        }

        winrt::com_ptr<ID3D11ShaderResourceView> srv = m_currentSRV;

        // 创建无边框、贴合整个窗口的 ImGui 窗口
